...
void FilterOnePole::print() {
  ...
  LOG(" Ylast: %f", Ylast);
  LOG(" X: %f", X);
  LOG(" ElapsedUS: %f", ElapsedUS);
  LOG(" TauSamps: %f", TauSamps);
  //LOG(" ampFactor: %f", ampFactor);
  LOG(" TauUS: %f\r\n", TauUS);
  //Serial.println("");
...
void FilterOnePole::test() {
  ...
  float nextupdateTime = HAL_GetTick()*1e-3;     //millis()*1e-3;
  ...
  while( true ) {
    float now = HAL_GetTick()*1e-3; //millis()*1e-3;
    ...
    if( now > nextupdateTime ) {
      ...
      LOG("inputValue: %f", inputValue);//Serial.print("inputValue: "); Serial.print( inputValue );
      LOG("\t high-passed: %f", hp.output()); //Serial.print("\t high-passed: "); Serial.print( hp.output() );  //Serial.print("\t high-passed: "); Serial.print( hp.output() );
      LOG("\t low-passed: %f\r\n", lp.output());  //Serial.print("\t low-passed: "); Serial.print( lp.output() );
    }
  }
}
...
void FilterOnePoleCascade::test() {
  ...
  LOG("testing filter with a rise time of %fs \n running filter speed loop ... ", rise);    //Serial.print( "testing filter with a rise time of ");
  //Serial.print( rise ); Serial.print( "s" );
  //Serial.print( "\n running filter speed loop ... ");
  
  float startTime, stopTime;
  
  startTime = HAL_GetTick()*1e-3;   //millis()*1e-3;
  for( long i=0; i<nLoops; ++i ) {
    myFilter.input( PI );   // use pi, so it will actually do a full calculation
  }
  stopTime = HAL_GetTick()*1e-3;  //millis()*1e-3;
  
  LOG("done, filter runs at %fhz \n filter value: %f", (float(nLoops) / (stopTime - startTime)), myFilter.output());
  ...
  stopTime = HAL_GetTick()*1e-3; //millis()*1e-3;
  LOG("done, fall time: %fs", stopTime-startTime);
  //Serial.print( "done, rise time: " ); Serial.print( stopTime-startTime );
  LOG("testing attenuation at f = 1/risetime");
  ...
  while( true ) {
    float now = 1e-3*HAL_GetTick();    ////1e-3*millis();
