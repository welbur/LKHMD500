...
float FilterTwoPole::input( float drive ) {
  ...
  long now = GetusTime();//micros();                      // get current time
...
void FilterTwoPole::test() {
  ...
  float nextupdateTime = 1e-6*float(GetusTime());      //(micros());

  float inputValue = 0;
  FilterTwoPole osc( 0.2, 4, 0);

  while( true ) {
    float now = 1e-6*float(GetusTime());      //(micros());        //(micros());
