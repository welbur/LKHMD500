...
osThreadId_t CHIPS_SPI1TransTaskHandle;
const osThreadAttr_t CHIPS_SPI1TransTask_attributes = {
	.name = "CHIPS_SPI1TransTask",
	.stack_size = 256 * 4,
...
osThreadId_t SPI2_WriteDataTaskHandle;
const osThreadAttr_t SPI2_WriteDataTask_attributes = {
	.name = "SPI2_WriteDataTask",
	.stack_size = 256 * 4,
...
void Start_CHIPS_SPI1TransTask(void *argument);
...
//	CHIPS_SPI1TransTaskHandle = osThreadNew(Start_CHIPS_SPI1TransTask, NULL, &CHIPS_SPI1TransTask_attributes);
	SPI2_WriteDataTaskHandle = osThreadNew(Start_SPI2_WriteDataTask, NULL, &SPI2_WriteDataTask_attributes);
...
//    
    ...
    {
      //LOGI("rec111 data : ");
  	  //for (int j = 0 ; j < re_arr_size ; j++) LOGI("%02X ", re_arr[j]);
  	  //LOGI(".......%d\r\n", re_arr_size);
      /*增加包头*/
  ...
  uint8_t sTxRxFlag = SpiTxRx_WAIT;
	/* Infinite loop */
  	for(;;)
  	{
        ...
        //LOGI("rxbuf : %d", rxbuf[0]);
        do {
          if (!HAL_GPIO_ReadPin(SPI2_CS_Port, SPI2_CS)) {//{LOGI("spi cs2 error....\r\n");continue;}
            if(HAL_SPI_Receive(&hspi2, rxbuf, 5, 20) != HAL_OK) {
              LOGE("spi MSP_SPI_read timeout....\r\n"); 
              //break;
            }
            while(HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY) {}
            //if(HAL_SPI_Receive(&hspi2, rxbuf, 5, 10) != HAL_OK) {LOGI("spi MSP_SPI_read timeout....\r\n"); break;}
            //while(HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY) {}
            //LOGI("cscscscs.....\r\n");
            //if (sTxRxFlag == SpiRx_COMPLETE) {{ LOGI("SpiRx_COMPLETE  read data\r\n"); break;}}
...
/**
 * @brief  spi1为主模式，通过SPI1与di/dq/rs485芯片通讯，每个板都有4颗芯片，通过cs选择
 * @param  argument: Not used
 * @retval None
 */
void Start_CHIPS_SPI1TransTask(void *argument)
{
	...
	for (;;)
	{
		...
		ISO1I813T_Read_Reg(CHIPH[D_I_Q_Chip_1].DChipID, INPDATA_ADDR);
		LOGI("Read Data from ISO1I813 chip complete.........%ld\r\n", HAL_GetTick());
...
void Start_SPI2_WriteDataTask(void *argument)
{
	...
	for (;;)
	{
		//if (MSP_SPI2_CS_STATUS()) break;
		HAL_SPI_MspInit(&hspi2);
		void *sTrans = SPITransfer_C_New(&CHIPH[D_I_Q_Chip_1], &hspi2, SET_SPIMODE_SLAVE);
		SPITransfer_C_Slave_Spi2_Transfer(sTrans, RxFlag, CHIPH[D_I_Q_Chip_1].DChipID);
		/*发送完spi数据后，需要加一条打印指令，要不会报错(因为主控也加了，要去都得去掉)*/
		//LOGI("write data to master complete.........%ld\r\n", HAL_GetTick());
		HAL_SPI_MspDeInit(&hspi2);
		osDelay(1);
