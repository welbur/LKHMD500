...
canopen_app_init(CANopenNodeSTM32* _canopenNodeSTM32) {
    ...
    if (CO == NULL) {
        ...
        LOG("Allocated %ld bytes for CANopen objects\n", heapMemoryUsed);
    }
...
canopen_app_resetCommunication() {
    ...
    LOG("CANopenNode - Reset communication...\n");
    ...
    if (err != CO_ERROR_NO) {
        LOG("Error: CAN initialization failed: %d\n", err);
        return 1;
    }
    ...
    if (err != CO_ERROR_NO) {
        LOG("Error: LSS slave initialization failed: %d\n", err);
        return 2;
    }
    ...
    if (err != CO_ERROR_NO && err != CO_ERROR_NODE_ID_UNCONFIGURED_LSS) {
        if (err == CO_ERROR_OD_PARAMETERS) {
            LOG("Error: Object Dictionary entry 0x%lX\n", errInfo);
        } else {
            LOG("Error: CANopen initialization failed: %d\n", err);
        }
        return 3;
    }
    ...
    if (err != CO_ERROR_NO) {
        if (err == CO_ERROR_OD_PARAMETERS) {
            ...
            LOG("Error: PDO initialization failed: %d\n", err);
        }
        return 4;
    }
    ...
    if (!CO->nodeIdUnconfigured) {
        ...
        LOG("CANopenNode - Node-id not initialized\n");
    }

    /* start CAN */
    CO_CANsetNormalMode(CO->CANmodule);

    LOG("CANopenNode - Running...\n");
    fflush(stdout);
...
void canopen_app_process() {
    ...
    if ((time_current - time_old) > 0) { // Make sure more than 1ms elapsed
        ...
        if (reset_status == CO_RESET_COMM) {
            ...
            CO_CANsetConfigurationMode((void*)canopenNodeSTM32);
            CO_delete(CO);
            LOG("CANopenNode Reset Communication request\n");
            canopen_app_resetCommunication(); // Reset Communication routine
        } else if (reset_status == CO_RESET_APP) {
            LOG("CANopenNode Device Reset\n");
