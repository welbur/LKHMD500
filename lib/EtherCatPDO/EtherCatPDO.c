...
//        if (N > 1) {
//            for (int i = 0; i < N; i++) {
//                insertCTOStr("DCM", ntoc[i], name, OBJ_Cache.BodyName[i]); //bodyname[i]);
//            }
//        } else 
...
{
    if (_etherCatH->RecvBufCnt <= 0 || (HAL_getTick_us() - _etherCatH->RecvLastTime < _etherCatH->t35chars))
    {
        return;
    }
    //LOG("current tick us : %ld\r\n", HAL_getTick_us() - _etherCatH->RecvLastTime);
...
{
    LOG_info("reStart Lan9255 ! "); // addMessage("Restart LAN9255 ! \r\n");

    FrameMsgID = addMsgID(FrameMsgID);
    byte Data[0];
    uint16_t DataLen = 0;
    EtherCatPDO_SendFrame(FrameMsgID, GetStatus_MsgType, Data, DataLen);
}
